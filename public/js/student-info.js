  ...
  fetch(`/api/user?childName=${encodeURIComponent(childName)}`)
    ...
    .then(userInfo => {
      document.getElementById('studentName').textContent =
        userInfo.childName || 'Name not provided';
      document.getElementById('parentName').textContent =
        userInfo.parentName || 'Parent name not provided';
      document.getElementById('parentPhone').textContent =
        userInfo.parentPhone || 'Phone not provided';
      document.getElementById('parentEmail').textContent =
        userInfo.parentEmail || 'Email not provided';
      document.getElementById('studentClass').textContent =
        userInfo.className || 'Class not provided';
      document.getElementById('classTime').textContent =
        userInfo.classSchedule || 'Schedule not provided';
    })
    .catch(error => {
